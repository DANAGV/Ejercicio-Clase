...
//*************	 Visual Studio 2017										******//
//*************		Fecha entrega: 26 Octubre 2018								******//
//*************		Practica 11										******//
//*************		Realiza parte de la coreografia de Thriller									******//
//*************		Manipulacion del teclado	
...
//'d' O 'D'
		...
		//'y' o 'Y'
			// eje 
		//moviento monito Izquierda 'g' o 'G'
		//moviento monito 'h' o 'H'
		//movimiento monito 'j' o' J'
		//Rodilla izq arriba 'b' abajo 'B'
	//Rodilla der arriba 'v' abajo 'V'
			//brazo izq delante 'n' atras 'N'
		// brazoder delante 'm' atras 'M'
		//cuello 'z' enfrente   'Z'  atras
		//giro monito der 'p'   izq 'P':
//*************											******//
//************************************************************//
//************************************************************//
...
float posX =0, posY = 2.5, posZ =-3.5, rotRodIzq = 0, rotRodDer = 0, rotBraIzq=0, rotBraDer=0, rotCintura=0, rotCuello=0;
float giroMonito = 0;

#define MAX_FRAMES 25  //que tan largo van a ser
int i_max_steps = 90;
int i_curr_steps = 0;
...
typedef struct _frame
{
	...
	float rotRodIzq; //variable para la rodilla Izq
	float rotRodDer; //variable para la rodilla Der
	float rotCodoDer; //variable para el codo Izq
	float rotCodoIzq; //variable para el codo Der
	float rotBraIzq; //variable para el brazo Izq
	float rotBraDer; //variable para el brazo Der
	float rotCintura; //variable para la cintura
	float rotCuello; //variable para el cuello

	float rotInc;

	float giroMonito;
	float giroMonitoInc, rotRodDerInc, rotRodIzqInc, rotBraDerInc, rotBraIzqInc, rotCinturaInc, rotCuelloInc, rotCodoDerInc, rotCodoIzqInc;
	
}FRAME; 

FRAME KeyFrame[MAX_FRAMES];
int FrameIndex=0;			//introducir datos
bool play=false;  //reproducir la animacion
int playIndex=0;
...
int w = 500, h = 500;
int frame=0,time,timebase=0; //el frame que se encuentra
char s[30];
...
void saveFrame ( void ) //guarda la animacion
{
	
	printf("frameindex %d\n",FrameIndex);			

	KeyFrame[FrameIndex].posX=posX;  //guarda las coordenadas
	KeyFrame[FrameIndex].posY=posY;
	KeyFrame[FrameIndex].posZ=posZ;

	KeyFrame[FrameIndex].rotRodIzq=rotRodIzq; //rotacion de la rodilla
	KeyFrame[FrameIndex].rotRodDer = rotRodDer; //rotacion de la rodilla
	KeyFrame[FrameIndex].rotBraIzq = rotBraIzq; //rotacion del brazo Izq
	KeyFrame[FrameIndex].rotBraDer = rotBraDer; //rotacion del brazo derecho
	KeyFrame[FrameIndex].rotCintura = rotCintura; //rotacion de la cintura
	KeyFrame[FrameIndex].rotCuello = rotCuello; //rotacion del cuello

	
	KeyFrame[FrameIndex].giroMonito=giroMonito; //rotacion
...
{
	posX=KeyFrame[0].posX; //da la posision, posicion final se da a la posicion actual
	...
	rotCintura = KeyFrame[0].rotCintura;
...
{
	KeyFrame[playIndex].incX = (KeyFrame[playIndex + 1].posX - KeyFrame[playIndex + 1].posX) / i_max_steps; //genera la interposicion	
	KeyFrame[playIndex].incY = (KeyFrame[playIndex + 1].posY - KeyFrame[playIndex + 1].posY) / i_max_steps;	//posicion final - la posicion inicial entre los frames
	KeyFrame[playIndex].incZ = (KeyFrame[playIndex + 1].posZ - KeyFrame[playIndex + 1].posZ) / i_max_steps;
	...
	KeyFrame[playIndex].rotCinturaInc = (KeyFrame[playIndex + 1].rotCintura - KeyFrame[playIndex].rotCintura) / i_max_steps;
	KeyFrame[playIndex].rotCuelloInc = (KeyFrame[playIndex + 1].rotCuello - KeyFrame[playIndex].rotCuello) / i_max_steps;
...
void monito() //se dibuja el monito
{
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix();//Cintura
			...
			glPushMatrix(); //Pie Derecho -->
				...
				glPushMatrix();
					glTranslatef(0, -0.5, 0);
					glRotatef(15 + rotRodDer, 1, 0, 0); //movimiento de la rodilla der
					glTranslatef(0, -0.75, 0);
					fig7.prisma(1.5, 0.5, 1, 0);
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			rotCintura += KeyFrame[playIndex].rotCinturaInc;
			rotCuello += KeyFrame[playIndex].rotCuelloInc;

			giroMonito+=KeyFrame[playIndex].giroMonitoInc;

			i_curr_steps++; //todo por arreglos
		}
