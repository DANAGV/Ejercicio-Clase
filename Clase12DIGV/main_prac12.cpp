...
//*************					Visual Studio 2017								*************//
//************* Este programa muestra el recorrido de un automovil que evade la ultima ruta y va directo al ultimo punto *************//
//	Del punto A (inicio) al punto B (primer vuelta), del B al punto C (segunda vuelta) y del punto C va al punto E *************//
//*************		Teclas de manipulacion   ****//
//**** "i" para iniciar el recorrido del auto ****//
//**   "o" para el movimiento del carrio atras y adelante    **//
//*     "l" movimiento del monito *//
//*     camaras "w" adelante,"s" atras, "a" izquierda, "d" derecha  */
//************************************************************//
...
void animacion()
{
	...
	//Movimiento del coche maquina estados
	if(g_fanimacion)
	{
		if(g_avanza)
		{
	...
	if(circuito)
	{
		...
		//editado para recorrer de C hasta E
		if(recorrido3)
		{
			rotKit = 215;    //rotacion del auto
			movKitZ -- ;
			if(movKitZ < 190) //limite en z punto E
			{
				movKitX--;
				if (movKitX < 370)   //limite en x punto E
				{
					movKitZ--;                            //moviento en Z
					if (movKitZ<-100)
					{
						movKitX--;                //movimiento en x punto c
						if (movKitX<15)   //limite en x
						{
							recorrido3 = false;
							recorrido5 = true;
						}
